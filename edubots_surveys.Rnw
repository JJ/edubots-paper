...
teachers.
% Antonio - Not yet, this is the abstract and not mentioned yet that teachers are the target (just in the title). We speak about the community. But anyway. It is not very relevant.
...
% Olia - we shoudl say here a few words about the two surveys, and their main difference. Also, whch RQs each survey addresses etc. 
% JJ - Yep... 
...
This survey asked the teachers, in addition to general information (such as gender, age, experience, etc), what are their feelings and preferences with regard to the use of communication tools with students, and even using bots or chatbots in the classroom.
...
%Olia - A chi-square test of independence was performed to examine the relation between instructors’ discipline, sector, and gender and any potential changes that occurred to the use of messaging apps due to the covid-19 pandemic. The relation between the latter variable and the instructors’ sector was significant, X2 (4, N = 282) = 9.598, p =0.048. The frequencies cross tabulated are given in Table X. [we can add a table here with the crosstabulations]. In effect, this finding indicates that how teachers responded to the use of messaging apps during the pandemic and in particular, whether they changed their habits in the use of apps for teaching purposes, was related to their sector (university vs vocational). 
