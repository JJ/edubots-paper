...
\cite{gachago2015crossing} precess. 
...
questions about enrollment and registration. Ask (Lancaster University)
...
progress. For instance, the work \cite{fernoagua2018intelligent} reported on
''eduAssistant'', a virtual teaching assistant chatbot developed inside the
...
features, and it enables users to exchange messages in different
formats (text, audio and video) \cite{fernoagua2018intelligent}. In
addition, Telegram run on all devices and operating
systems. ''eduAssistant'' chatbot acts as an automatic agent
...
Authors in \cite{sjostrom2018designing} have proposed a conceptual
...
previous studies, examining the design of chatbots in higher education, as well as a content 
...
In this line, our previous work \cite{MoraChatbots2021} aimed at analysing the expectations of students on this regard, whereas the present study is focused on the other key actor in this challenge, i.e. educators.
