...
% TODO: introduction
The introduction of new technologies in the classroom involves a combination of teacher training, student enthusiasm and possibly acquiring new skills by both. Some technologies are readily adapted, but others require longer time for full adoption. In most cases, the magnitude of the uptake of the new technology impinges on the collaboration of all the parties using it. Hence, examining the users’ attitudes and their opinions is an important first step in boosting this collaboration and reaping the benefits of the technology.  Recently, a surge in the use of synchronous instant messaging tools in higher education such as WhatsApp, Telegram and Facebook Messenger took place. Some messaging applications have built in chatbots that are able to support a synchronous conversation between the different parties \cite{differ.chat}, while other chatbots are developed as standalone systems and can possibly be attached to a messaging application using an API (Application Program Interface). 
...
attitudes will be the objective of RQ3 in this paper.
...
and why the attitudes of the teaching community towards them have to be
...
Specifically, this survey asked the teachers, in addition to general information (such as gender, age, experience, etc), what  their feelings and preferences are with regard to the use of communication tools with students, and even using bots or chatbots in the classroom.
In addition the survey asked about the most relevant \textit{Interaction media features} for the teachers with respect to the potential tool to use in class. These features are:
\begin{itemize}
...
Regarding Gender grouping, both gender agree in their uses, so it seems not relevant at all. When looking at the plot containing the sector differentiation it is interesting ot observe how university professors prefer in a higher proportion to use them for grades and FAQs in comparison with Vocational/High School teachers. This seems reasonable as the frequency of seeing the students is smaller at the University so, automating the grade notification for continuous evaluation could seem much more comfortable using an automated tool than using the traditional platform. Furthermore, the high student/professor ratios that university professors have to deal with could be another reason to have a higher number of them willing to use chatbots for FAQs. This could ameliorate the number of misunderstandings and email overload that so many students might generate.

The results, when grouping by experience, confirm the previous observation that experienced professors are not afraid of changes and are willing to accept new technological challenges, at least, in the same way that younger professors are. From this concrete plot it is interesting as well the fact that very few young/inexperience professors are willing to use chatbots for other uses. This reinforces the hypothesis thrown at the beginning of this section and, as these professors have less experience, they also have less field of view to think about ways of using chatbots. 
...
As it appears in the responses presented above, teachers’ responses are distributed between positive and negative in relation to the media features: analytics, connectivity, and hidden phone, while for the features: familiarity, horizontally, sustainability and officiality the majority provided a positive response, designating their preference to these features. On the other hand, the majority of teachers seems to consider not valuable the presence of official formation, pluggability, unidirectionality and synchronization in the interaction media features of messaging apps. In other words, the majority of the teachers seem to prefer to use messaging apps to which they are already familiar, and which have been granted by their academic institution, normally also accompanied with technical support by the university’s staff and which do not require any actions on the behalf of the teachers for maintenance. In addition, the majority of the teachers value more the messaging apps in which there is no hierarchy in the communication level among the different users (i.e., teachers, students), and when the use of the app does not require or reveal their personal phone number. Last, the majority of the teachers value the most messaging apps that provide analytics in a form of a dashboard. 

From a qualitative point of view, these results show comsensus in a few key aspects:

\begin{itemize}
 \item Professors want to have support from their corresponding institution. This is probably due to the strict European data regulations and to avoid the overhead required to sing up and deleting students from year to year.
 
 \item Professors need sustainability. The main reason is that implementing changes is expensive in terms of effort and prone to errors in the first years of implementing new procedures, therefore, it is logical that if chatbots or messaging platforms are to be introduced, they want this change to be as permanent as possible.
...
\end{itemize}
%Olia: be consistent in the terms being used, we refer to teachers, then to professors. I would replace the word "professors" above with the word "teachers". 

A chi-square test of independence was performed to examine the potential relationship among the various features. The statistically significant correlations are given in Table \ref{tab:interactionfeatures}. As indicated from the chi square results, the presence of the analytics feature in messaging apps, has been found to be positively correlated with the presence of features such as, connectivity, familiarity, official formation, pluggability and sustainability. This means, that teachers who prefer to use an app (or tool) that provides analytics about students’ actions and/or performance, most probably also prefer this app to be able to be connected to other applications, offer different plugins to the teachers and be an app with which the teachers are already familiar, while receiving some training from the university about its use and do not have to worry about its maintenance. In addition, the hidden phone feature was found to be correlated with the features: official formation (i.e., the university offers some training courses or talks about the tool/app) and unidirectionality (i.e., only the teacher can publish information in the channel). This can be expected, as teachers might prefer not to reveal personal data, such as their personal phone number, when the tool is being introduced by the university and when the level of communication is one-way (i.e., from the teacher to the students). Furthermore, pluggability has been found to be correlated with official formation, implying that teachers who prefer pluggability as a feature in a messaging app/tool, they should also prefer to having this tool/app being properly and officially introduced by the university and offer some training about its usage. 
...
Finally, a two-step cluster analysis was conducted, with a log-likelihood distance measure adopted, to explore how the cases could be grouped based on their preferences of specific interaction media features, and which features have larger predictor importance for the clustering (see Fig. \ref{figure:predictor}). Two clusters occurred from the analysis: cluster 1 (n=78, 60.9\%) was comprise of instructors who did not value interaction media pluggability, connectivity, official formation, but valued interaction media analytics, familiarity, officiality, while cluster 2 (n=50, 39.1\%) was comprise of instructors from which the majority did not value media analytics, official formation, familiarity, but valued pluggability, connectivity, and officiality. The formation of the two clusters demonstrates different trends that appear to exist in teachers’ responses, based on their most preferred interaction media features. In other words, there is a group of teachers who value the most certain features, such as being familiar with an application, having access to analytics, and having the support from the university when using the application, while a second group of teachers seems to value mostly the features of pluggability, the capability of the app to connect with other apps, and receiving training from the university before/while using the app for teaching purposes. Interaction media pluggability appears to have the most important predictor importance in the clustering of cases (predictor importance = 1.0), whilst media unidirectionality was the least important factor (predictor importance = 0.04). The cluster quality is fair, but not a good one (silhouette measure of cohesion and separation). 
