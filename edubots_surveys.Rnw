...
% JJ - feel free to add one 
% Antonio - The thing is I don't catch why are we saying that, since it is supossed that we should 'defend' the use of chatbots (from the project perspective, at least). Or if we don't want to defend them, at least not saying that they are useless most of times. XD
% Moreover, considering that in the following paragraphs we are trying to 'sell' them.
...
• RQ1 - Are teachers already using messaging apps in their classes? 
• RQ2 - Which kind of chatbots would teachers find useful in their classes? 
• RQ3 - Which kind of interaction do teachers prefer with their students? 
The remainder of the paper is organized as follows: Next is an overview of what current research has found about the use of messaging applications, including chatbots, in the classroom. The methodology used in the survey is presented in Section 3, and the results of the survey are presented next in Section 4. Finally, we discuss these results and conclude with a series of recommendations in the section that closes the paper.
...
In the present work we aim to complement the aforementioned study with an analysis of teacher attitudes; teachers are the ones that actually would need to use, 
% Olia-I believe we should avoid refereeing to “attitudes”. Also here, it is not mentioned that we explore their views towards the use of chat tools/ messaging apps. 
% Antonio - 'deploy' instead implement? Since in Computer Sciences implement and program are synonyms...
% JJ - deployment would be done by the university or whoever publishes the system. Deployment and implementation are two different things, anyway
% Antonio - Ok, but what I said is that 'implement' is a synonym of 'program' in this scope, and we write both in the same sentence as different things.
% And deploy does not make sense in this context. I will simply say use - JJ
