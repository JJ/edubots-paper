...
]{\inits{J.J.M.}\fnm{Juan J.} \snm{Merelo}}
\author[
  addressref={aff1},
  email={pacv@ugr.es}   % email address
]{\inits{P.A.C.}\fnm{Pedro A.} \snm{Castillo}}
\author[
  addressref={aff2},
  email={amorag@ugr.es}
]{\inits{A.M.M}\fnm{Antonio M.} \snm{Mora}}
\author[
  addressref={aff1},
  email={fbarranco@ugr.es}
]{\inits{F.B.}\fnm{Francisco} \snm{Barranco}}
\author[
  addressref={aff3},
  email={n.h.abbas@leeds.ac.uk}
]{\inits{N.A.}\fnm{Noorhan} \snm{Abbas}}

% Antonio - Olia and Alberto are missing. ;)
...
teachers based mainly in Spain and Spanish speaking countries, looking
at what they think about the introduction of messaging platforms and
chatbots, what their needs are, what kind of use cases they see for
...
gender, experience, and other factors, such as the kind of discipline
...
resources in informal learning settings \cite{panah2020study}.  The
...
students \cite{abbas2021onlinechat} without affecting students'
ownership and control that is vital to advance their learning
\cite{gachago2015crossing}. This is why examining the role of MIM in higher education is still a challenge, and why the attitudes of the teaching community towards them has to be examined, as we do in this paper.
...
The use of conversational agents or chatbots in higher education is
...
this sense, \cite{Pimmer2019} carried out a series of experiments with
...
Several higher education chatbots' evaluation studies were
undertaken. For instance, an evaluation review study presented by
...
quality metric such as language, subject matter and platform. Another
...
questions about enrollment and registration. Ask
L.U. \cite{LancasterUniversity_2019} answers students' frequently
...
other.  \cite{ranoliya2017chatbot} proposed a generic chatbot for
university students that is able to answer their frequently asked
questions; for some time, the University of Granada had ELVIRA as a
chatbot in their main page \cite{MOREO20129}. Besides being able to
answer pre-established (frequent) questions, it did so from the main
University of Granada using an inset character, voiced by a real
person, who lip-synched the answers. The emphasis in this case,
...
interactive, using actual messaging platforms. In most cases, using
the inset search engine was able to give you more up-to-date and
accurate answers.
...
Telegram messaging platform. In this study, the Telegram platform was
chosen because it is easy to use, students are familiar with its
features and it enables students to exchange messages in different
...
between the teacher-content-student, facilitating real-time feedback
loops and providing a personalised learning experience relevant to
students' skills and knowledge. Using this chatbot, educators can
create interactive instances in their lectures where they pose
questions to their students and the chatbot assists students who need
further help by giving those more hints and reporting to their
educator's dashboard \cite{fernoagua2018intelligent}. This can help
educators locate those students that need their attention and send
them more educational resources relevant to their academic
attainment.

Despite its (arguably) successful implementation in different higher education institutions, over all in the fringes of educational activity, its implementation or deployment is not trivial.
Some authors \cite{sjostrom2018designing} have proposed a conceptual
architecture for the adoption teaching-oriented chatbots in higher education.  This
conceptual architecture is based on a systematic literature review of
previous studies examining the design of chatbots in higher education
and a content analysis of student emails and discussion forum posts of
...
Messenger) which confirms with \cite{hobert2019you} and
\cite{fernoagua2018intelligent} studies. In addition,
\cite{sjostrom2018designing} argued that a conceptualision of
learners' questions could aid designers in integrating the appropriate
types of questions that the chatbots should support for different
courses.  \cite{coronado2018cognitive} proposed agents that store
learning materials to be provided on demand to
students. \cite{crockett2017predicting} reported on tutoring systems,
...
higher education is a special domain where according to
\cite{hobert2019you}, specific pedagogical factors such as learning
success or increased motivation are more important.  Therefore, to
develop efficient chatbots for higher education, all stakeholders'
...
The descriptive analysis in this case is done using a Chi-square test. For the different variables considered, some statistical significance was found for Gender, Sector, and academic Discipline. The values found with the test are: $p-value=0.0034$ for Gender, $p-value=0.0440$ for Sector, and $p-value=0.0180$ for Discipline.
...
@
    ...
    \multicolumn{1}{|l|}{Female} & \textbf{-2.776} & \textbf{2.637} & \textbf{2.028}  & -1.159 \\ 
    \multicolumn{1}{|l|}{} & 14.48\% & \textbf{27.48\%} &  16.46\% & 5.75\% \\ \hline
    \multicolumn{1}{|l|}{Male}   & \textbf{2.776} & \textbf{-2.637} & \textbf{-2.028} & 1.159 \\ 
    \multicolumn{1}{|l|}{} &  8.08\% &  \textbf{15.34\%} & 9.19\% &  3.21\% \\ \hline
    \end{tabular}%
    }
    \end{minipage}
    \begin{minipage}[c]{0.45\textwidth}
...
@
    ...
    \cline{2-5}
                                 & Did/Do & Didn't/Do & Safer  & Didn' t/Don't \\ \hline
    \multicolumn{1}{|l|}{University} & -1.895 & 0.397 & 0.049  &  \textbf{2.679}\\ 
    \multicolumn{1}{|l|}{University} & 5.81\% & 0.54\% & 0.01\%  &  \textbf{26.48\%}\\ \hline
    \multicolumn{1}{|l|}{Vocational} & -1.895 & -0.397 & -0.049 & \textbf{-2.679}\\ 
    \multicolumn{1}{|l|}{Vocational} & 11.88\% & 1.10\% & 0.02\% & \textbf{54.17\%}\\ \hline
    \end{tabular}%
    }
    \end{minipage}
    \begin{minipage}[c]{0.45\textwidth}
...
@
    ...
    \cline{2-5}
                                 & Did/Do & Didn't/Do & Safer & Didn' t/Don't \\ \hline
    \multicolumn{1}{|l|}{Biomed/Sciences} & -1.202 &  0.715 & 1.424 & -0.635 \\ 
    \multicolumn{1}{|l|}{Biomed/Sciences} & 3.12\% &  2.32\% & 9.34\% & 1.99\% \\ \hline
    \multicolumn{1}{|l|}{Hum/Social} &  \textbf{-2.869} & \textbf{2.069} &  1.592 & 0.274\\ 
    \multicolumn{1}{|l|}{Hum/Social} &  16.58\% & \textbf{18.13\%} &  10.88\% & 0.35\%\\ \hline
    \multicolumn{1}{|l|}{Tech/Engineering} &  \textbf{3.382} &  \textbf{-2.322} &  \textbf{-2.461} & 0.254\\ 
    \multicolumn{1}{|l|}{Tech/Engineering} &  11.92\% &  11.80\% &  \textbf{13.43\%} & 0.15\%\\ \hline
    \end{tabular}%
    }
    \end{minipage}
\end{center}
...
The authors want to thank to the educators who participated in the survey.
