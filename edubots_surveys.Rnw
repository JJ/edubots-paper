...
\title{Chatbots in the classroom: an analysis from the teacher's perspective}
...
Introducing new technologies such as messaging platforms, and chatbots attached to it, in higher education, requires careful consideration of all options, but above all, an examination of the attitudes of the affected community. In this paper we have surveyed the opinions of tertiary education teachers based mainly in Spain and Spanish speaking countries, looking at what they think of the introduction of messaging platforms, what their needs are, what kind of use cases they see for them, and in what kind of environment they think its use will be valuable. We will examine how and when these attitudes vary across gender, experience, and other factors, such as the kind of discipline they are teaching.
\end{abstract}
...
% TODO: introduction
Introduction of new technologies in the classroom needs a combination of teacher education, as well as student willingness and skills. Some technologies are readily adapted, some of them take a long time to be adopted. In most cases, success or failure of the introduction of a 

A \textit{chatbot} is a program that allows a computer to communicate in a similar way as a human does \cite{Gong2008}. Indeed, most of them would be, apparently, close to passing a classic Turing test \cite{moor2003turing}, since they are able to answer almost any speaker’s question fluently, and even ask other questions to the human.

The technology behind chatbots \cite{Gong2008,bradesko2012} has become an usual element in our everyday life, having played an important role in the development of many fields, including education and online tutoring \cite{clarizia2018,Smutny2020}. These automatic systems ease personalized learning, adapting individuals' pace of learning, who get personalized online tutoring outside the classroom. Thus, chatbots can significantly contribute to providing interactive learning experience as well as individual attention \cite{agarwal2020}.
...
Moreover, using an automated system such as a chatbot allows the teacher to detect those topics that are most in demand, and on which he or she should consequently place more emphasis in class.

% Maybe move it to a different place later
As a starting point, we have analyzed the responses to a survey from XXX teachers asking about their attitudes towards the use of chatbots in their classes. We are interested in discussing implications and the impact for the development of future technology-enhanced tools and the Education institutions policies regarding the following research questions:
\begin{itemize}
\item{ RQ1 - Are teachers already using messaging apps in their classes? }
\item{ RQ2 - Which kind of chatbots are they interested in? }
\item{ RQ3 - Regarding technology acceptance, are there any differences regarding age, gender,  or years of experience in Education? }
\item{ RQ4 - } 
\end{itemize}
...
We collected responses from a total of XXX teachers from University and Vocational Education from Spain ...
...
According to the results summarized in Fig. \ref{figure:chatbots_use_gender_age_exp}, there are no significant differences between male and female teachers. Regarding the instant messaging apps, teachers mainly use Whatsapp (40\%-50\%), the most popular messaging app in Spain. Similar numbers are shown for the use of the app provided by their institutions, Universities or Education System Administration. Finally, about 20\% use Telegram. The distribution of messaging apps per age are not significant for the Whatsapp and apps provided by their institutions. However, younger teachers use also Telegram with more than 25\%, a percentage that falls to about 10\% for teachers that are 55 or older. The third column compares the use of these apps considering the number of years of experience. One interesting result is that, contrarily to the idea of technology adaptation issues when considering older teachers, about 65\% of teachers with more than 25 years of experience use the platforms provided by their institutions while the percentage goes down to less than 50\% for teachers with 6-15 years of experience.

\begin{figure}[t]
\begin{center}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/gender_provided.eps}
    \end{minipage}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/age_provided.eps}
    \end{minipage}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/experience_provided.eps}
    \end{minipage}
    
    \vspace{2mm}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/gender_whatsapp.eps}
    \end{minipage}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/age_whatsapp.eps}
    \end{minipage}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/experience_whatsapp.eps}
    \end{minipage}
    
    \vspace{2mm}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/gender_telegram.eps}
    \end{minipage}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/age_telegram.eps}
    \end{minipage}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/experience_telegram.eps}
    \end{minipage}
\end{center}
    \caption{Use of chatbots in class: distributions per gender and age. Teachers were specifically asked about two popular instant messaging apps (Whatsapp and Telegram), and the use of platforms already provided by their institutions different than these two ones.}
\label{figure:chatbots_use_gender_age_exp}
\end{figure} 

Results in Fig. \ref{figure:chatbots_use_sector_discip} do not show differences in the use of instant messaging apps between teachers from University and vocational education. In general, most teachers prefer messaging apps provided by their own institutions. With respect to specific disciplines, Engineering and Technology teachers are more active in their use, although it is also relevant the amount of teachers from Humanities that answered they used these apps in their classes (around 60\% use the apps provided by their institutions).

\begin{figure}[t]
\begin{center}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/sector_provided.eps}
    \end{minipage}
    \begin{minipage}[t]{0.50\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/discipline_provided.eps}
    \end{minipage}
    
    \vspace{2mm}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/sector_whatsapp.eps}
    \end{minipage}
    \begin{minipage}[t]{0.50\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/discipline_whatsapp.eps}
    \end{minipage}
    
    \vspace{2mm}
    \begin{minipage}[t]{0.30\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/sector_telegram.eps}
    \end{minipage}
    \begin{minipage}[t]{0.50\textwidth}
	    \centering
 	    \includegraphics[width=\textwidth]{figures/discipline_telegram.eps}
    \end{minipage}
\end{center}
